...
- [ ] 2단계 - `seed.py`: 이 repo를 단독으로 시연할 수 있도록 데모 데이터 삽입 스크립트.
- [ ] 3단계 - `monitor.py`: 전체 목록/건수/요약 통계 조회 함수 + 표 형태 콘솔 렌더링.
- [ ] 4단계 - `main.py`: 메뉴 루프 — `[R]` 입력 시 재조회 및 재렌더링, `[0]` 종료.
- [ ] 5단계 - `tests/test_monitor.py`: 조회 함수 단위 테스트 (`:memory:` DB 사용).
